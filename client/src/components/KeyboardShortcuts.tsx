  ...
  const formatKeys = (keys: string[]) => {
    return keys.map((key, index) => (
      <span key={`${key}-${index}`} className="inline-flex items-center">
        <Badge 
          variant="outline" 
          className="px-2 py-1 text-xs font-mono bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-600"
        >
          {key}
        </Badge>
        ...
        <div className="space-y-6">
          ...
          <div className="mt-6 pt-4 border-t border-slate-200 dark:border-slate-700">
            <div className="flex items-center justify-between text-sm text-slate-500 dark:text-slate-400">
              <span>Press <Badge variant="outline" className="mx-1 px-2 py-1 font-mono">Esc</Badge> to close this overlay</span>
